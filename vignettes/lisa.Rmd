...
```
We then reorder the matrix so the order is consistent with the elements of resL
```{r reordermatrix}
indx <- setNames(1:nrow(SimMat), rownames(SimMat))[rownames(resL[[1]])]
SimMat <- SimMat[indx, indx]
```
Finally, using this matrix, we can create an \code{hclust} object for clustering and plot the clustered heatmap. The plotting of the dendogram is controlled by the parameter \code{show_dendrogram}. 
```{r plottfsclustered}
