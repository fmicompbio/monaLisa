...
#reorder matrix so that WMs are in the same order as in the elements of resL 
indx <- setNames(1:nrow(SimMat), rownames(SimMat))[rownames(resL[[1]])]
SimMat <- SimMat[indx, indx]
```
Using this matrix, we can create a \code{hclust} object for clustering and plot the clustered heatmap. The plotting of the dendgram is controlled by the parameter \code{show_dendrogram}. 
```{r plottfsclustered}
