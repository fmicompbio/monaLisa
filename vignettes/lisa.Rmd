...
We use an example data set compare chromatin accessibility between liver and lung samples.
...
The \code{findMotifHits} function can be used to get the binding site of each transcription factor across a given set of genomic regions, 
which in out case is the ATAC-seq peaks. The \code{get_numberOfTFBS_perSeqName} subsequently generates our predictor matrix which 
