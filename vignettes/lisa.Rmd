...
# Selecting Transcription Factors with Stability Selection

Randomized Lasso stability selection proposed by Meinshausen and Buehlman (REF) applies the lasso regression on several subsamples of the data using a weakness parameter that adds additional variation to the lambda parameter and allows for more correlated variables to be selected together. The authors argue for this as a way of slightly overcomming the problem of lasso only selecting variables that are not correlated, and performing better in a noisy dataset depending on how strongly one sets the weakness parameter. SHah and Blah further improved error bounds (REF).

Here we demonstrate the use of randomized lasso stability selection to select transcription factors (TFs) that are likely to explain changes in chromatin accessibility from ATAC-seq data. One can also use stability selection on any other type of data like ChIP-seq of specific histone marks for example. We are given a vector of log-fold changes (logFC) in chromatin accessibility between two conditions that we consider to be our response vector. The predictor matrix will consist of the number of transcription factor bindng sites (TFBS) that each TF has across the peaks. The example data set comes from the Bing Ren lab (REF) and was processed in the LAB (REF) from which we obtained the bam files for ATAC-seq from mouse postnatal (P0) liver and lung tissues. Each condition had 2 replicates, and the peak calling was done per condition using MACS2 (REF). The peaks from both conditions were merged and used to calculate counts of the ATAC for all samples with QuasR (REF). Counts per million (CPM) were calculated per sample and averaged across replicates. The logFC was calculated as log2(CPM(Liver)+8) - log2(CPM(Lung)+8).

Only distal autosomal peaks were kept since promoter peaks tend not to change so much in accessibility, and we are interested in selecting TFs that likely explain logFC in enhancer regions. The `.rds` files used in lisa's example dataset consist of peaks that had an absolute logFC greater than 2 and leass than 2.2. This was done to reduce the number of peaks and speed up runtime to demonstrate the functions used in `lisa` that apply randomized lasso stability selection (REF).



### Load Data

We use an ATAC-seq dataset from ENCODE (REF) as example to demonstrate the use of stability selection to select TFs that are likely to explain log-fold changes in chromatin accessibility. UPDATE HERE on details of teh response and peaks variables. 
...
We can now run randomized lasso stability selection to select TFs that are likely to explain the log-fold changes in accessibility (response vector).
\
