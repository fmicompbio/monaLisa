...
# # if using a Windows OS, parallelize using parLapply (see stabs::stabsel for more details)
# library(parallel)
# cl <- makeCluster(2)
# se <- randLassoStabSel(x = TFBSmatrix, y = gr$logFC_liver_vs_lung,
...
Next, we visualize the correlation structure of the TFBS matrix and selected TF motifs. We can see that some TFs have similar binding sites across the genomic regions and that randomized lasso stability selection does reasonably well in selecting TF motifs from all given ranges of correlation.
```{r TFBScor_selected}
