...
Lasso stability selection implements the lasso regression multiple times on subsamples of the data. With the randomized lasso, a weakness parameter is additionally used to randomly vary the lambda parameter of the lasso regression. Although the main appeal of stability selection is in cases where the number of predictors exceeds the number of observations, it also performs better in the variable selection in noisy data [@StabSel].
...
predictor_matrix <- unclass(table(factor(seqnames(hits), levels = seqlevels(hits)),
...
# peaks variable
w <- names(peaks)[rowSums(predictor_matrix) == 0]
if (length(w) > 0) {
    peaks <- peaks[-w]
    response <- response[-w]
}
...
```{r run_stability, eval=TRUE}
stabs <- randLassoStabSel(x = predictor_matrix, y = response,
                            weakness = 0.8, cutoff = 0.7, PFER = 2,
                            mc.cores = 2)
```
...
par(mfrow = c(1, 1), mar = c(5, 4, 4, 2) + 3)
plotMotifDirectionality(se = stabs)

# plot correlation on TFBS matrix of selected TFs
# ... select predictors for selected TFs and calculate correlation
cutoff <- metadata(stabs)$stabsel.params.cutoff
selected <- stabs$selProbCutoff0.7
sel_matrix <- predictor_matrix[, selected]
