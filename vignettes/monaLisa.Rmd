...
among them REDUCE [@reduce] and ISMARA [@ismara]. In `monaLisa`, we aim to
...
- In **binned motif enrichment analysis** (`calcBinnedMotifEnrR()`, see
...
[@homer]. This approach has for example been used in @ginno2018 and @barisic2019.
...
`monaLisa` uses a robust regression approach (stability
...
For this example, we assume that you already have extracted a set of peaks. The 
sequences of the peak regions are stored in a `DNAStringSet` (`peak_seqs`), and 
additionally each peak is associated with a numeric value (e.g., the change of 
methylation between two conditions, stored in the `peak_change` vector), that 
will be used to bin the regions before finding motifs enriched in each bin. 
...
- *pearsonResid*: Pearson residuals, a measure for motif enrichments calculated
  ...
  correspond to $z$ values ($z = (o - \mu)/\sigma$), assuming that the numbers 
  of regions in a bin that contain a motif are independent Poisson random 
  variables with mean and variance $e$.
...
`monaLisa` contains a file with genomic coordinates (mouse mm10 assembly) of 
...
all transcription factor motifs that have an $-log_{10} FDR$ of at least 8.0 in 
any bin (corresponding to an $FDR < 10^{-8}$). FDR values are stored in the 
`negLog10Padj` assay:  

```{r plottfs}
# select strongly enriched motifs
sel <- apply(assay(se, "negLog10Padj"), 1, 
             function(x) max(abs(x), 0, na.rm = TRUE)) > 8.0
sum(sel)
seSel <- se[sel, ]
...
of the `"pearsonResid"` assay. As many transcription factor binding motifs 
are similar to each other, it is also helpful to show the enrichment heatmap 
clustered by motif similarity. To this end, we first calculate all pairwise 
motif similarities (measured as maximum Pearson correlation of all possible 
shifted alignments). This can be quickly calculated for the few selected motifs 
using the `motifSimilarity` function. For many motifs, this step may take a 
while, and it may be useful to parallelize it using the `BPPARAM` argument 
...
`monaLisa` provides two functions for performing binned motif enrichment 
analysis (`calcBinnedMotifEnrHomer` and `calcBinnedMotifEnrR`). 
`calcBinnedMotifEnrR` implements the binned motif enrichment analysis 
...
base probabilities), for example by calculating log-odds scores (see `?toPWM` 
...
`calcBinnedMotifEnrR` takes PWMs as a `PWMatrixList` object to scan for motif 
hits. `calcBinnedMotifEnrHomer` on the other hand takes a motif text file with 
PPMs, and requires the user to have `Homer` installed to use it for the binned 
motif enrichment analysis. Here, we show how one can get motif PFMs from 
`r Biocpkg("JASPAR2018")` and convert them to a `Homer`-compatible text file 
with PPMs (`dumpJaspar`) and vice versa (`homerToPFMatrixList` ), and how to 
convert a `PFMatrixList` to a `PWMatrixList` for use with `calcBinnedMotifEnrR` 
or `findMotifHits`: 
...
sekm <- readRDS(system.file("extdata", "se_6mers.rds", package = "monaLisa"))
```
...
selkm <- apply(assay(sekm, "negLog10Padj"), 1, 
               function(x) max(abs(x), 0, na.rm = TRUE)) > 0.5
sum(selkm)
sekmSel <- sekm[selkm, ]
```
...
strongly enriched motifs do not seem to correspond to an annotated motif. 

```{r}
pfmSel <- rowData(seSel)$motif.pfm
sims0 <- motifKmerSimilarity(x = pfmSel, kmerLen = 6)
sims <- sims0[, rownames(sekmSel)]
# simsrevcomp <- sims0[, as.character(reverseComplement(DNAStringSet(rownames(sekmSel))))]
# sims <- pmax(sims, simsrevcomp)
dim(sims)
...
As mentioned `monaLisa` can also be used to scan sequences for motifs. Here is 
...
library(JASPAR2018)
library(TFBSTools)
pfms <- getMatrixByID(JASPAR2018, c("MA0885.1", "MA0099.3", "MA0033.2", 
                                    "MA0037.3", "MA0158.1"))
pwms <- toPWM(pfms)
pwms
name(pwms)
