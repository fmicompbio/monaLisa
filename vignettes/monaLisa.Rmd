...
therefore requires a local installation of Homer, and `calcBinnedKmerEnr()`
...
to avoid length bias, for example by using a fixed-size region around the
midpoint of each region of interest using `resize`. In addition, the resized
...
Now that all regions are of the same length, we can directly extract the
...
We can check for differences in sequence composition between bins using
the `plotBinDiagnostics` function. "GCfrac" will plot the distributions
...
correction in `calcBinnedMotifEnrR` should be able to handle it.
...
selected motifs using the `motifSimilarity` function. For many motifs, this step
...
analysis (`calcBinnedMotifEnrR` and `calcBinnedMotifEnrHomer`). 
