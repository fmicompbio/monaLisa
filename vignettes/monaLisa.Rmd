...
## Convert between motif text file for `Homer` and `TFMatrixList` in `R`

`calcBinnedMotifEnr` is a wrapper function for `calcBinnedMotifEnrR` and `calcBinnedMotifEnrHomer`. `calcBinnedMotifEnrR` implements the binned motif enrichment analysis in `R`, similarly to `Homer` and does not require the user to have the `Homer` tool pre-installed. For more information on that function and how it resembles the `Homer` tool see the function details. This function takes a `PWMatrixList` as input, to scan for motif hits. `calcBinnedMotifEnrHomer` on the other hand takes a motif text file of position probability matrices (PPMs) as input, and requires the user to have `Homer` installed to use it for the binned motif enrichment analysis. Here, we show how one can convert the `TFMatrixList` to a text file and vice versa. 


The `Homer` motif file contains the probability each letter (A, C, G, T) has at a specific position of a motif. The probabilities per position sum up to 1. `homerToPFMatrixList` reads in this file and constructs a `PFMatrixList`. Each entry is multiplied by the provided number of observations. The `dumpJaspar` function on the other hand, takes a specified data base like `JASPAR2018` as input, extracts the position frequency matrices (PFMs), adds a count of 1 to each entry, and calculates position probability matrices per motif before writing that into a file format suitable for `Homer`. 

```{r}
# get JASPAR PFMs to demonstrate
pfms <- getMatrixSet(JASPAR2018,
                     opts = list(matrixtype = "PFM",
                                 tax_group = "vertebrates"))

# convert pfms to Homer motif file
tmp <- tempfile()
...
# getting a pseudo-count of 1 first
sweep(x = as.matrix(pfms[[1]]) + 1, MARGIN = 2, 
      STATS = colSums(as.matrix(pfms[[1]]) + 1), FUN = "/")
sweep(x = as.matrix(pfms_ret[[1]]), MARGIN = 2, 
      STATS = colSums(as.matrix(pfms_ret[[1]])), FUN = "/")
```
