...
specificities such as `JASPAR2018`, also available from Bioconductor. Genomic
...
# Quick example: Identify enriched motifs{#quick}
...
`se` is a `SummarizedExperiment` with `assays` *p*, *FDR*, *enr*, *log2enr*,
*sumForegroundWgtWithHits* and *sumBackgroundWgtWithHits*, each containing a matrix
with motifs (rows) by bins (columns). The values are:  

- *p*: the raw P value ($-\log_{10} p$) of a given motif enrichment in a given
  bin. Each P value results from an enrichment calculation comparing occurrences
  of each motif in the bin to its occurrences in all other bins as a background.  
- *FDR*: Same as *p* but adjusted for multiple testing using the method of
  Benjamini and Hochberg, 1995 (`p.adjust(..., method="fdr"`).  
- *enr*: Motif enrichments, calculated as: $enr = (o - e) / \sqrt e$, where $o$
  and $e$ are the observed and expected numbers of regions in the bin containing a
...
# Detailed example: Finding TFs enriched in differentially methylated regions{#binned}
...
se <- calcBinnedMotifEnrHomer(gr = lmrsel, b = bins,
                              genomedir = "/work/gbioinfo/DB/genomes/mm9",
...
As the Jaspar database contains many similar motifs, it is helpful to show the
enrichment heatmap clustered by motif similarity. To this end, we first need
to calculate all pairwise motif similarities (measured as maximum Pearson
correlation of all possible shifted alignments). This can be quickly calculated
for the selected motifs using the `motifSimilarity` function. For many motifs,
this step may take a while, and it may be useful to parallelize it using
the `BPARAM` argument (e.g. to run on `n` parallel threads using the multi-core
...
`seSel`. Using this matrix, we can create an `hclust` object for clustering and
plot the clustered heatmap. The plotting of the dendogram is controlled by
the parameter `show_dendrogram`: 
```{r plottfsclustered}
...
set.seed(6698)
s <- sample(1:length(pfms), size = 100)
pfms <- pfms[s]
pwms <- toPWM(pfms) # for searching

# get TFBS on given GRanges (peaks)
peakSeq <- BSgenome::getSeq(BSgenome.Mmusculus.UCSC.mm10, gr)
...
# check order
all(rownames(TFBSmatrix) == names(gr))

# remove TFs with 0 binding sites in all regions
...
TFBSmatrix <- cbind(fracGC, TFBSmatrix)
TFBSmatrix[1:6, 1:6]
```
