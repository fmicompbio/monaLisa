...
- *negLog10Padj*: Same as *p* but adjusted for multiple testing using the method of
...
sel <- apply(assay(se, "negLog10Padj"), 1, function(x) max(abs(x))) > 8.0
sum(sel)
seSel <- se[sel, ]
...
```{r predictorMatrix}
# get PWMs (random subset of 200 vertebrate TFs from Jaspar)
...
We can also visualize the selection probabilities of the selected TF motifs, optionally multiplied by the sign of the correlation to the response vector, to know how the TF relates to the change of accessibility (`directional` parameter). Note that although one can vary the `selProbMinPlot` parameter which sets the selection probability cutoff, it is best to re-run randomized lasso stability selection with the new cutoff, as this influences the other parameters model uses internally. See @StabSel for more details.

```{r plotSelProbs, fig.width=15, fig.height=7}
plotSelectionProb(se, directional = TRUE)
```

Next, we visualize the correlation structure of the TFBS matrix and selected TFs. We can see that some TFs have similar binding sites across the genomic regions.

<!--, and that randomized lasso stability selection does reasonably well in selecting motifs from all given ranges of correlation, even highly correlated motifs. -->
