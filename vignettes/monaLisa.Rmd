...
`calcBinnedMotifEnrHomer` implements 
...
and `calcBinnedKmerEnr`(see section \@ref(binnedkmers)) 
...
`r Biocpkg("monaLisa", vignette="selecting_motifs_with_randLassoStabSel.html",
label="the stability selection vignette")` in `r Biocpkg("monaLisa")`, uses a
robust regression approach (stability selection, @StabSel) to predict what
transcription factors can explain experimental measurements, for example changes
in chromatin accessibility between two conditions. Also this approach allows to
correct for sequence composition. In addition, similar motifs have to "compete"
with each other to be selected.
...
you already have extracted a set of peaks. The sequences of the peak regions are
stored in a `Biostrings::DNAStringSet` object (`peak_seqs`), and additionally
each peak is associated with a numeric value (e.g., the change of methylation
between two conditions, stored in the `peak_change` vector), that will be used
to bin the regions before finding motifs enriched in each bin.
...
- *pearsonResid*: Standardized Pearson residuals, a measure of motif enrichment
  ...
  @agresti2007, section \@ref(runBinned)). 
...
any bin (corresponding to a $FDR < 10^{-4}$). FDR values are stored in the
`negLog10Padj` assay:
...
calculate all pairwise motif similarities (measured as maximum Pearson
...
depletion of KLF family motifs is likely an example of an incorrect result,
