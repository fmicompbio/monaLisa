...
We will now construct the transcription factor binding site (TFBS) matrix for all motifs (from a database like JASPAR) across the peak regions. We use the `findMotifHits` function to get the TF hits. This matrix will be the predictor matrix in our regression.

As mentioned, this framework offers the flexibility to add other predictors to compete against the TFs for selection. Here, we add GC and observed/expected CpG content.
...
hits <- findMotifHits(query = pwms, subject = peaks, min.score = 6.0,
                      method = "matchPWM", genome = genome,
                      BPPARAM = BiocParallel::MulticoreParam(2))
...
We can now run randomized lasso stability selection to select TFs that are likely to explain the log-fold changes in accessibility. 

```{r stabSelTFs}
# select TFs
set.seed(123)
se <- randLassoStabSel(x = TFBSmatrix, y = logFC, mc.cores = 2)
se

# number of selected TFs
sum(se$selProbCutoff0.8)
```
...
We can also visualize the selection probabilities of the selected TFs, as well as the selection probability multiplied by the sign of the correlation to the logFC vector (response vector), to know how the TF relates to the logFC.

```{r plotSelProbs, fig.width=15, fig.height=7}
par(mfrow = c(1, 2), mar = c(5, 4, 4, 2) + 3)
plotSelectionProb(se)
plotMotifDirectionality(se, selProbMinPlot = 0.7)
```
...
sel <- colnames(se)[se$selProbCutoff0.8]
TFBSmatrixCorSel <- TFBSmatrixCor[sel, sel]

# heatmap
colF = colorRamp2(c(-1, 0, 1), c("blue", "white", "red"))

Heatmap(TFBSmatrixCor, show_row_names = FALSE, show_column_names = FALSE, 
        name = "Pear. Cor.", column_title = "All TFs", col = colF)

pfmsSel <- pfms[match(sel, name(pfms))]
...
Heatmap(TFBSmatrixCorSel, show_row_names = TRUE, show_column_names = TRUE, 
        name = "Pear. Cor.", column_title = "Selected TFs", col = colF, 
        right_annotation = hmSeqlogo)
```
We examine the peaks that have hits for a selected TF of interest, ordered by the biggest absolute logFC values.
...
nm <- names(sort(abs(logFC[i]), decreasing = TRUE))
