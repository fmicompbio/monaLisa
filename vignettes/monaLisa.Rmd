...
[@homer]. This approach was first introduced in @ginno2018 and subsequently applied in e.g. @barisic2019.
...
- In **randomized Lasso stability selection** (`monaLisa::randLassoStabSel`, see
...
sequences of the peak regions are stored in a `Biostrings::DNAStringSet` (`peak_seqs`), and 
...
- *pearsonResid*: Pearson residuals, a measure for motif enrichments calculated
  as $enr = (o - \mu)/\sigma$), where $\mu$ are the expected counts and $\sigma$
  the standard deviation in counts based on a hypergeometric distribution that assumes
  no difference between foreground and background.
