...
getKmerFreq <- function(seqs, kmerLen = 4, MMorder = 2, pseudoCount = 1) {
    ...
    lp_long  <- log2(oligonucleotideFrequency(seqs, width = MMorder)      %>% {colSums(. + pseudoCount)} %>% {./sum(.)})
    lp_short <- log2(oligonucleotideFrequency(seqs, width = MMorder - 1L) %>% {colSums(. + pseudoCount)} %>% {./sum(.)})
