...
test_that("motifSimilarity works as expected", {
    ...
    expect_is(res1 <- motifSimilarity(x = pfmL, y = NULL, method = "R", BPPARAM = BiocParallel::MulticoreParam(3), verbose = TRUE), "matrix")
    expect_is(res2 <- motifSimilarity(x = pfmL, y = NULL, method = "R", BPPARAM = BiocParallel::MulticoreParam(2), verbose = TRUE), "matrix")
    expect_is(res3 <- motifSimilarity(x = pfmL, y = pfmL, method = "R", BPPARAM = BiocParallel::MulticoreParam(2), verbose = TRUE), "matrix")
    expect_is(res4 <- motifSimilarity(x = pfmL, y = pfmL, method = "R", verbose = TRUE), "matrix")
    expect_is(res5 <- motifSimilarity(x = tmpf, y = NULL, method = "R", verbose = TRUE), "matrix")
...
test_that("motifKmerSimilarity works as expected", {
    ...
    expect_is(res1 <- motifKmerSimilarity(x = pfmL, kmerLen = 4, verbose = TRUE), "matrix")
